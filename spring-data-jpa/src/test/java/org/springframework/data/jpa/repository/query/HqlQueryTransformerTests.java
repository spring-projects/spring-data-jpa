...
class HqlQueryTransformerTests {
	...
	void demo(String query) {
		...
		//
		//
		var transformed = new QueryParsingEnhancer(new HqlQueryParser(query)).applySorting(sort);
	...
	void detectsAliasCorrectly() {
		...
		assertThat(new QueryParsingEnhancer(new HqlQueryParser(QUERY)).detectAlias()).isEqualTo("u");
		assertThat(new QueryParsingEnhancer(new HqlQueryParser(SIMPLE_QUERY)).detectAlias()).isEqualTo("u");
		assertThat(new QueryParsingEnhancer(new HqlQueryParser(COUNT_QUERY)).detectAlias()).isEqualTo("u");
	...
	@Disabled("Must extend char set for tokens")
	@Test
	// DATAJPA-736
