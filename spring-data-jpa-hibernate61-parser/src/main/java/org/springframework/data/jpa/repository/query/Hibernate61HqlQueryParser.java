 ...
 * {@link HqlParser} and a custom {@link Hibernate61HqlQueryTransformer}.
 *
 * @author Greg Turnquist
 * @author Mark Paluch
 * @since 3.1
 */
class Hibernate61HqlQueryParser extends JpaQueryParserSupport {

	Hibernate61HqlQueryParser(String query) {
		super(query);
	}
	...
	/**
	 * Use the {@link Hibernate61HqlQueryTransformer} to transform the original query into a query with the {@link Sort}
	 * applied.
	 *
	 * @param parsedQuery
	...
	protected List<JpaQueryParsingToken> applySort(ParserRuleContext parsedQuery, Sort sort) {
		return new Hibernate61HqlQueryTransformer(sort).visit(parsedQuery);
	}

	/**
	 * Use the {@link Hibernate61HqlQueryTransformer} to transform the original query into a count query.
	 *
	 * @param parsedQuery
	...
	protected List<JpaQueryParsingToken> doCreateCountQuery(ParserRuleContext parsedQuery,
		...
		return new Hibernate61HqlQueryTransformer(true, countProjection).visit(parsedQuery);
	}

	/**
	 * Run the parsed query through {@link Hibernate61HqlQueryTransformer} to find the primary FROM clause's alias.
	 *
	 * @param parsedQuery
	...
	protected String doFindAlias(ParserRuleContext parsedQuery) {
		...
		Hibernate61HqlQueryTransformer transformVisitor = new Hibernate61HqlQueryTransformer();
		transformVisitor.visit(parsedQuery);
		return transformVisitor.getAlias();
	}

	/**
	 * Use {@link Hibernate61HqlQueryTransformer} to find the projection of the query.
	 *
	 * @param parsedQuery
	 * @return
	 */
	...
	/**
	 * Use {@link Hibernate61HqlQueryTransformer} to detect if the query uses a {@code new com.example.Dto()} DTO
