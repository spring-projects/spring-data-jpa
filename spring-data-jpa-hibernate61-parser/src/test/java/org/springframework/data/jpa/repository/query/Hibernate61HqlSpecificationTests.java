 ...
 * @since 3.1
 */
class Hibernate61HqlSpecificationTests {
	...
	void joinExample1() {
		...
		Hibernate61HqlQueryParser.parseQuery("""
				SELECT DISTINCT o
	...
	void hqlQueries() {
		...
		Hibernate61HqlQueryParser.parseQuery("from Person");
		Hibernate61HqlQueryParser.parseQuery("select local datetime");
		Hibernate61HqlQueryParser.parseQuery("from Person p select p.name");
		Hibernate61HqlQueryParser.parseQuery("update Person set nickName = 'Nacho' " + //
				"where name = 'Ignacio'");
		Hibernate61HqlQueryParser.parseQuery("update Person p " + //
				"set p.name = :newName " + //
				"where p.name = :oldName");
		Hibernate61HqlQueryParser.parseQuery("update Person " + //
				"set name = :newName " + //
				"where name = :oldName");
		Hibernate61HqlQueryParser.parseQuery("update versioned Person " + //
				"set name = :newName " + //
				"where name = :oldName");
		Hibernate61HqlQueryParser.parseQuery("insert Person (id, name) " + //
				"values (100L, 'Jane Doe')");
		Hibernate61HqlQueryParser.parseQuery("insert Person (id, name) " + //
				"values (101L, 'J A Doe III'), " + //
				"(102L, 'J X Doe'), " + //
				"(103L, 'John Doe, Jr')");
		Hibernate61HqlQueryParser.parseQuery("insert into Partner (id, name) " + //
				"select p.id, p.name " + //
				"from Person p ");
		Hibernate61HqlQueryParser.parseQuery("select p " + //
				"from Person p " + //
		...
		Hibernate61HqlQueryParser.parseQuery("select p " + //
				"from Person p " + //
				"where p.id = 1L");
		Hibernate61HqlQueryParser.parseQuery("select c " + //
				"from Call c " + //
		...
		Hibernate61HqlQueryParser.parseQuery("from Phone ph " + //
				"where ph.type = LAND_LINE");
		Hibernate61HqlQueryParser.parseQuery("select java.lang.Math.PI");
		Hibernate61HqlQueryParser.parseQuery("select 'Customer ' || p.name " + //
				"from Person p " + //
				"where p.id = 1");
		Hibernate61HqlQueryParser.parseQuery("select sum(ch.duration) * :multiplier " + //
				"from Person pr " + //
				"join pr.phones ph " + //
				"join ph.callHistory ch " + //
				"where ph.id = 1L ");
		Hibernate61HqlQueryParser.parseQuery("select year(local date) - year(p.createdOn) " + //
				"from Person p " + //
				"where p.id = 1L");
		Hibernate61HqlQueryParser.parseQuery("select p " + //
				"from Person p " + //
				"where year(local date) - year(p.createdOn) > 1");
		Hibernate61HqlQueryParser.parseQuery("select " + //
				"	case p.nickName " + //
				"	when 'NA' " + //
				"	then '<no nick name>' " + //
				"	else p.nickName " + //
		...
		Hibernate61HqlQueryParser.parseQuery("select nullif(p.nickName, p.name) " + //
				"from Person p");
		Hibernate61HqlQueryParser.parseQuery("select " + //
				"	case" + //
				...
				"	end " + //
				"from Person p");
		Hibernate61HqlQueryParser.parseQuery("select coalesce(p.nickName, '<no nick name>') " + //
				"from Person p");
		Hibernate61HqlQueryParser.parseQuery("select coalesce(p.nickName, p.name, '<no nick name>') " + //
				"from Person p");
		Hibernate61HqlQueryParser.parseQuery("select p " + //
				"from Person p " + //
				"where size(p.phones) >= 2");
		Hibernate61HqlQueryParser.parseQuery("select concat(p.number, ' : ' , cast(c.duration as string)) " + //
				"from Call c " + //
				"join c.phone p");
		Hibernate61HqlQueryParser.parseQuery("select substring(p.number, 1, 2) " + //
				"from Call c " + //
				"join c.phone p");
		Hibernate61HqlQueryParser.parseQuery("select upper(p.name) " + //
				"from Person p ");
		Hibernate61HqlQueryParser.parseQuery("select lower(p.name) " + //
				"from Person p ");
		Hibernate61HqlQueryParser.parseQuery("select trim(p.name) " + //
				"from Person p ");
		Hibernate61HqlQueryParser.parseQuery("select trim(leading ' ' from p.name) " + //
				"from Person p ");
		Hibernate61HqlQueryParser.parseQuery("select length(p.name) " + //
				"from Person p ");
		Hibernate61HqlQueryParser.parseQuery("select locate('John', p.name) " + //
				"from Person p ");
		Hibernate61HqlQueryParser.parseQuery("select abs(c.duration) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select mod(c.duration, 10) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select sqrt(c.duration) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select cast(c.duration as String) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select str(c.timestamp) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select str(cast(duration as float) / 60, 4, 2) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select c " + //
				"from Call c " + //
				"where extract(date from c.timestamp) = local date");
		Hibernate61HqlQueryParser.parseQuery("select extract(year from c.timestamp) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select year(c.timestamp) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select var_samp(c.duration) as sampvar, var_pop(c.duration) as popvar " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select bit_length(c.phone.number) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select c " + //
				"from Call c " + //
				"where c.duration < 30 ");
		...
		Hibernate61HqlQueryParser.parseQuery("select p " + //
				"from Phone p " + //
				"where type in :types");
		Hibernate61HqlQueryParser.parseQuery("select distinct p " + //
				"from Phone p " + //
		...
		Hibernate61HqlQueryParser.parseQuery("select distinct p.person " + //
				"from Phone p " + //
				"join p.calls c " + //
				"where 50 > all (" + //
				"	select duration" + //
		...
		Hibernate61HqlQueryParser.parseQuery("select distinct pr, ph " + //
				"from Person pr, Phone ph " + //
				"where ph.person = pr and ph is not null");
		Hibernate61HqlQueryParser.parseQuery("select distinct pr1 " + //
		...
		Hibernate61HqlQueryParser.parseQuery("select p " + //
				"from Payment p ");
		Hibernate61HqlQueryParser.parseQuery("select d.owner, d.payed " + //
				"from (" + //
				"  select p.person as owner, c.payment is not null as payed " + //
				"  from Call c " + //
				"  join c.phone p " + //
				"  where p.number = :phoneNumber) d");
		Hibernate61HqlQueryParser.parseQuery("select distinct pr " + //
				"from Person pr " + //
		...
		Hibernate61HqlQueryParser.parseQuery("select distinct pr " + //
				...
				"where ph is null " + //
				"   or ph.type = :phoneType");
		Hibernate61HqlQueryParser.parseQuery("select pr.name, ph.number " + //
				"from Person pr " + //
		...
		Hibernate61HqlQueryParser.parseQuery("select a, ccp " + //
				"from Account a " + //
				"join treat(a.payments as CreditCardPayment) ccp " + //
				"where length(ccp.cardNumber) between 16 and 20");
		Hibernate61HqlQueryParser.parseQuery("select c, ccp " + //
				"from Call c " + //
				"join treat(c.payment as CreditCardPayment) ccp " + //
				"where length(ccp.cardNumber) between 16 and 20");
		Hibernate61HqlQueryParser.parseQuery("select longest.duration " + //
				"from Phone p " + //
				"left join lateral (" + //
		...
		Hibernate61HqlQueryParser.parseQuery("select ph " + //
				"from Phone ph " + //
		...
		Hibernate61HqlQueryParser.parseQuery("select ch " + //
				"from Phone ph " + //
				"join ph.callHistory ch " + //
				"where ph.id = :id ");
		Hibernate61HqlQueryParser.parseQuery("select value(ch) " + //
				"from Phone ph " + //
				"join ph.callHistory ch " + //
				"where ph.id = :id ");
		Hibernate61HqlQueryParser.parseQuery("select key(ch) " + //
				"from Phone ph " + //
				"join ph.callHistory ch " + //
				"where ph.id = :id ");
		Hibernate61HqlQueryParser.parseQuery("select key(ch) " + //
				...
				"join ph.callHistory ch " + //
				"where ph.id = :id ");
		Hibernate61HqlQueryParser.parseQuery("select entry(ch) " + //
				"from Phone ph " + //
				"join ph.callHistory ch " + //
				"where ph.id = :id ");
		Hibernate61HqlQueryParser.parseQuery("select sum(ch.duration) " + //
				"from Person pr " + //
				"join pr.phones ph " + //
				"join ph.callHistory ch " + //
				"where ph.id = :id " + //
				"  and index(ph) = :phoneIndex");
		Hibernate61HqlQueryParser.parseQuery("select value(ph.callHistory) " + //
				"from Phone ph " + //
				"where ph.id = :id ");
		Hibernate61HqlQueryParser.parseQuery("select key(ph.callHistory) " + //
				"from Phone ph " + //
				"where ph.id = :id ");
		Hibernate61HqlQueryParser.parseQuery("select p " + //
				"from Person p " + //
		...
		Hibernate61HqlQueryParser.parseQuery("select pr " + //
				"from Person pr " + //
				"where pr.phones[max(indices(pr.phones))].type = 'LAND_LINE'");
		Hibernate61HqlQueryParser.parseQuery("select p.name, p.nickName " + //
				"from Person p ");
		Hibernate61HqlQueryParser.parseQuery("select p.name as name, p.nickName as nickName " + //
				"from Person p ");
		Hibernate61HqlQueryParser.parseQuery("select new org.hibernate.userguide.hql.CallStatistics(" + //
				"	count(c), " + //
				"	sum(c.duration), " + //
				"	min(c.duration), " + //
				"	max(c.duration), " + //
				...
				")  " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select new map(" + //
				...
				"join c.phone p " + //
				"group by p.number ");
		Hibernate61HqlQueryParser.parseQuery("select new list(" + //
				"	p.number, " + //
				"	c.duration " + //
				")  " + //
				"from Call c " + //
				"join c.phone p ");
		Hibernate61HqlQueryParser.parseQuery("select distinct p.lastName " + //
				"from Person p");
		Hibernate61HqlQueryParser.parseQuery("select " + //
				"	count(c), " + //
				"	sum(c.duration), " + //
				...
				"	avg(c.duration)  " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select count(distinct c.phone) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select p.number, count(c) " + //
				"from Call c " + //
				"join c.phone p " + //
				"group by p.number");
		Hibernate61HqlQueryParser.parseQuery("select p " + //
		...
		Hibernate61HqlQueryParser.parseQuery("select count(c) filter (where c.duration < 30) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select p.number, count(c) filter (where c.duration < 30) " + //
				"from Call c " + //
				"join c.phone p " + //
				"group by p.number");
		Hibernate61HqlQueryParser.parseQuery("select listagg(p.number, ', ') within group (order by p.type,p.number) " + //
				"from Phone p " + //
				"group by p.person");
		Hibernate61HqlQueryParser.parseQuery("select sum(c.duration) " + //
				"from Call c ");
		Hibernate61HqlQueryParser.parseQuery("select p.name, sum(c.duration) " + //
				"from Call c " + //
				"join c.phone ph " + //
				"join ph.person p " + //
				"group by p.name");
		Hibernate61HqlQueryParser.parseQuery("select p, sum(c.duration) " + //
				"from Call c " + //
				"join c.phone ph " + //
				"join ph.person p " + //
				"group by p");
		...
		Hibernate61HqlQueryParser.parseQuery("select p.name from Person p " + //
				"union " + //
				"select p.nickName from Person p where p.nickName is not null");
		Hibernate61HqlQueryParser.parseQuery("select p " + //
				"from Person p " + //
				"order by p.name");
		Hibernate61HqlQueryParser.parseQuery("select p.name, sum(c.duration) as total " + //
				"from Call c " + //
				"join c.phone ph " + //
				"join ph.person p " + //
				"group by p.name " + //
