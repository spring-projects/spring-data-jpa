...
footnote:[If the default persistence provider Hibernate is used, beware that in `Fetch` mode all attributes marked with `@FetchType.EAGER` are loaded although they are not listed in the `@EntityGraph.attributePaths` or `@NamedEntityGraph.attributeNodes` lists (see the according hint in the http://docs.jboss.org/hibernate/orm/current/userguide/html_single/Hibernate_User_Guide.html#fetching-strategies-dynamic-fetching-entity-graph[Hibernate manual]).]
