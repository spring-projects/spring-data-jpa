...
* `stream()`: Return a `Stream<T>` to process results as a stream rather than a materialized `Collection` (see xref:repositories/query-return-types-reference.adoc#return-type.stream[`Stream` semantics]).
